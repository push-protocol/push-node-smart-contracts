...
async function deployAllLocalhost() {
  ...
  let vNodeCount = EnvLoader.getPropertyAsNumber("VNODE_COUNT", 10);
  console.log('VNODE_COUNT=', vNodeCount);

  if(--vNodeCount >= 0) await RegisterUtil.registerNode(hre, pushToken.address, validatorCt.address, owner,
  ...
  let sNodeCount = EnvLoader.getPropertyAsNumber("SNODE_COUNT", 8);
  console.log('SNODE_COUNT=', sNodeCount);

  // register storage nodes
  if(--sNodeCount >= 0) await RegisterUtil.registerNode(hre, pushToken.address, validatorCt.address, owner,
  ...
  let aNodeCount = EnvLoader.getPropertyAsNumber("ANODE_COUNT", 5);
  console.log('ANODE_COUNT=', aNodeCount);

  // NEW A1
  if(--aNodeCount >= 0) await RegisterUtil.registerNode(hre, pushToken.address, validatorCt.address, owner,
